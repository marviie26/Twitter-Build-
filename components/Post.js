...
import { HeartIcon , ChatBubbleOvalLeftIcon ,  ShareIcon ,  ArrowPathRoundedSquareIcon, TrashIcon} from '@heroicons/react/24/outline'
        ...
        <div className='grid grid-cols-[48px,1fr] gap-4'>
          ...
          <div>
            <div className='block sm:flex gap-1'>
              ...
              <div className='flex'>
                <p className='text-slate-300 '>@{post?.tag} &nbsp;·&nbsp;</p>
            ...
            <div className='flex justify-between text-[20px] mt-4 w-[80%]'>
              ...
              <div className='flex gap-1 items-center'>
                <ChatBubbleOvalLeftIcon className='hoverEffect w-7 h-7 p-1' onClick={(e) => {
                  e.stopPropagation()
                  openModal()
                }} />
              ...
              {session.user.uid !== post?.id ? (
                <ArrowPathRoundedSquareIcon className='hoverEffect w-7 h-7 p-1' />
              ) : (
                <TrashIcon className='hoverEffect w-7 h-7 p-1'
                  onClick={(e) => {
                    e.stopPropagation();
              ...
              <div className='flex gap-1 items-center'
                ...
                {liked ? <HeartIcon className='hoverEffect w-7 h-7 p-1 text-pink-700' />
                  : <HeartIcon className='hoverEffect w-7 h-7 p-1' />}
  
                {likes.length > 0 && (<span className={`${liked && "text-pink-700"} text-sm`}>{likes.length}</span>)}
              </div>
  
              <ShareIcon className='hoverEffect w-7 h-7 p-1' />
            </div>
